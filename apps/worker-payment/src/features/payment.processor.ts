...
export class PaymentProcessor extends WorkerHost {
    ...
    async process(job: Job<any>): Promise<any> {
        ...
        this.logger.log(`[BullMQ] Bắt đầu xử lý thanh toán cho giao dịch: ${referenceCode}`);
        ...
        const acquireLock = await redisClient.set(lockKey, 'LOCKED', 'PX', 10000, 'NX');

        if (!acquireLock) {
            this.logger.warn(`[BullMQ] Phát hiện xử lý song song cho đơn hàng ${referenceCode}. Skip job.`);
        ...
        try {
            ...
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                if (!dbTx) {
                    throw new Error(`Không tìm thấy giao dịch thanh toán trong hệ thống: ${referenceCode}`);
                }

                // LỚP BẢO VỆ 2: Chống lặp giao dịch (Idempotency check)
                if (dbTx.status === ('SUCCESS' as BillingTxStatus)) {
                    this.logger.warn(`[BullMQ] Giao dịch ${referenceCode} đã được xử lý thành công trước đó.`);
            ...
            if (result.status === 'PROCESSED') {
                ...
                this.logger.log(`[Success] Nâng cấp thành công gói ${result.tier} cho User ${result.userId} (Ref: ${referenceCode})`);
            }

            return result;
        } catch (err: any) {
            this.logger.error(`[Error] Thất bại khi xử lý đơn hàng ${referenceCode}: ${err.message}`);
