...
export class SubscriptionSchedulerService {
  ...
  async sweepExpiredSubscriptions() {
    ...
    try {
      ...
      this.logger.log(
        `[Cron] Tìm thấy ${expiredSubs.length} tài khoản quá hạn gia hạn.`,
      );
      ...
      for (const sub of expiredSubs) {
        ...
        if (updated) {
          ...
          this.logger.log(
            `[Cron] Đã hạ cấp thành công tài khoản User ${sub.userId} về gói FREE do hết hạn.`,
          );
        } else {
          this.logger.log(
            `[Cron] Bỏ qua hạ cấp cho User ${sub.userId} vì gói cước đã được cập nhật/thay đổi.`,
          );
        }
      }
    } catch (err: any) {
      this.logger.error(
        `[Cron] Lỗi nghiêm trọng khi quét dọn tài khoản hết hạn: ${err.message}`,
      );
    }
  }
  ...
  async reconcilePendingTransactions() {
    this.logger.log(
      "[Cron] Khởi chạy tác vụ dọn dẹp đơn hàng PENDING quá hạn...",
    );
    ...
    try {
      ...
      if (expiredTxs.length > 0) {
        this.logger.log(
          `[Cron] Phát hiện ${expiredTxs.length} giao dịch PENDING quá hạn (2 giờ). Cập nhật trạng thái thành EXPIRED.`,
        );
        for (const tx of expiredTxs) {
          ...
          this.logger.log(
            `[Cron] Đơn hàng ${tx.referenceCode} đã tự động chuyển sang trạng thái EXPIRED.`,
          );
        }
      } else {
      ...
      this.logger.error(
        `[Cron] Lỗi khi thực hiện dọn dẹp đơn hàng quá hạn: ${err.message}`,
      );
    }
  }
}
