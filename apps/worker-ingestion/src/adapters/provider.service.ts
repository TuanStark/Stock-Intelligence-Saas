  ...
  async getQuote(symbol: string): Promise<NormalizedQuote> {
    for (let i = 0; i < this.quoteProviders.length; i++) {
      ...
      try {
        ...
        this.logger.warn(
          `Provider ${provider.name} failed to get quote for ${symbol}. ${i < this.quoteProviders.length - 1 ? "Falling back to next provider..." : "No more providers available."}`,
        );
  ...
  async getHistorical(
    ...
    for (let i = 0; i < this.historicalProviders.length; i++) {
      ...
      try {
        ...
        this.logger.warn(
          `Provider ${provider.name} failed to get historical data for ${symbol}. ${i < this.historicalProviders.length - 1 ? "Falling back..." : "No more providers."}`,
        );
        if (i === this.historicalProviders.length - 1) {
          throw new Error(
            `All providers failed to get historical for ${symbol}`,
          );
        }
      }
    }
