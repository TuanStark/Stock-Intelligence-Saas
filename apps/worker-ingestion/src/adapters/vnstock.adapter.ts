...
export class VnStockAdapter implements IMarketDataProvider {
  ...
  private readonly dchartUrl = 'https://dchart-api.vndirect.com.vn/dchart/history';
  ...
  async getQuote(symbol: string): Promise<NormalizedQuote> {
    ...
    try {
      ...
      this.logger.warn(`VNDIRECT DChart failed to get quote for ${symbol}: ${error.message}. Falling back to Yahoo Finance...`);
    }
    ...
    try {
      ...
      this.logger.error(`Yahoo Finance fallback also failed for ${symbol}: ${error.message}`);
  ...
  async getHistorical(
    ...
    try {
      ...
      this.logger.warn(`VNDIRECT DChart failed to get historical for ${symbol}: ${error.message}. Falling back to Yahoo Finance...`);
    }
    ...
    try {
      ...
      this.logger.error(`Yahoo Finance historical fallback failed for ${symbol}: ${error.message}`);
  ...
  async getCompanyProfile(symbol: string): Promise<NormalizedCompany> {
    ...
    try {
      ...
      return {
        ...
        pe: sp.trailingPE || (p.regularMarketPrice && ks.trailingEps ? p.regularMarketPrice / ks.trailingEps : 0),
        pb: ks.priceToBook || 0,
        eps: ks.trailingEps || 0,
        outstandingShares: ks.sharesOutstanding || p.sharesOutstanding || 0,
      };
    } catch (error: any) {
      this.logger.error(`Yahoo Finance getCompanyProfile failed for ${symbol}: ${error.message}`);
      
      // Secondary fallback: Return a generic basic profile to prevent bootstrap failures
      this.logger.warn(`Returning basic mock profile as final safety measure for ${cleanSymbol}`);
      return {
        symbol: cleanSymbol,
