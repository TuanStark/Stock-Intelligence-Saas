...
export class FinancialDataProcessor extends WorkerHost {
  ...
  async process(job: Job<FinancialIngestionJobPayload>): Promise<any> {
    ...
    this.logger.log(`[Queue Worker] Processing financial ingestion job ${job.id} for ${symbol} (Action: ${name})...`);
    ...
    try {
      ...
      this.logger.error(`Failed to ingest financial data for ${symbol}: ${(error as Error).message}`, (error as Error).stack);
