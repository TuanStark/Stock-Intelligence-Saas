...
export class StockProcessingProcessor extends WorkerHost {
  ...
  async process(job: Job<StockProcessingJobPayload>): Promise<any> {
    ...
    this.logger.log(
      `Processing technical indicators & signals job for ${symbol}...`,
    );
    ...
    try {
      ...
      if (candles.length === 0) {
        this.logger.warn(
          `No historical candles found in DB for ${symbol}. Cannot process indicators.`,
        );
      ...
      this.logger.error(
        `Failed to process technical signals for ${symbol}: ${err.message}`,
        err.stack,
      );
