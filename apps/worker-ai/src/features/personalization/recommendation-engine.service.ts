...
export class RecommendationEngineService {
  ...
  async computePersonalizedScores(userId: string): Promise<any> {
    this.logger.log(`🤖 Starting Recommendation Score computation for user ${userId}…`);
    ...
    portfolios.forEach(p => p.positions.forEach(pos => {
    ...
    const portfolioSymbols = new Set(dbPositions.map(p => p.instrument.symbol.toUpperCase()));
    const watchlistSymbols = new Set(dbWatchlistItems.map(w => w.instrument.symbol.toUpperCase()));
    ...
    const upsertPromises = instruments.map(async inst => {
      ...
      const sectorWeight = inst.sectorId ? (preferredSectors[inst.sectorId] || 0) : 0;
    ...
    this.logger.log(`✅ Completed Recommendation Score computation for user ${userId}`);
    return { success: true };
  }
}
