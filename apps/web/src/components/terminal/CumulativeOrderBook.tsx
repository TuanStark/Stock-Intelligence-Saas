...
export const CumulativeOrderBook: React.FC<CumulativeOrderBookProps> = ({
  bids,
  asks,
  tc,
}) => {
        ...
        <table className="w-full text-[10.5px] border-collapse font-mono">
          <thead>
            <tr className="text-text-muted border-b border-[#151a24]/80 h-5 font-bold">
              <th className="text-left pb-0.5 font-bold uppercase text-[9px] w-2/5">
                KL
              </th>
              <th className="text-right pb-0.5 font-bold uppercase text-[9px] w-1/5 pr-1">
                Giá mua
              </th>
              <th className="text-left pb-0.5 font-bold uppercase text-[9px] w-1/5 pl-1">
                Giá bán
              </th>
              <th className="text-right pb-0.5 font-bold uppercase text-[9px] w-2/5">
                KL
              </th>
            </tr>
          </thead>
          <tbody>
            {bids.map((bid, idx) => {
              ...
              return (
                <tr
                  key={idx}
                  className="h-6 hover:bg-white/2 transition-colors relative"
                >
                  <td className="relative text-left text-white font-medium pl-1">
                    ...
                    <span className="relative z-10">
                      {formatCurrency(bid.volume)}
                    </span>
                  </td>

                  <td
                    className={`${bidPriceColor} font-extrabold text-right pr-1 relative z-10`}
                  >
                    {formatCurrency(bid.price)}
                  </td>

                  <td
                    className={`${askPriceColor} font-extrabold text-left pl-1 relative z-10`}
                  >
            ...
            {bids
              ...
              .map((bid, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex-1 h-full flex flex-col justify-end items-center gap-1 group relative"
                  >
                    <div
                      style={{ height: `${h}%` }}
                    ...
                    />
                    <span className="text-[8px] text-emerald-500 font-bold scale-90">
                      {formatCurrency(bid.price)}
                    </span>
                    ...
                    <div className="absolute bottom-full mb-1 hidden group-hover:block bg-[#141923] border border-[#2d3748]/60 text-[8px] text-white p-1 rounded z-30 shadow-xl whitespace-nowrap">
                      Mua: {formatCurrency(bid.price)} | KL:{" "}
                      {formatCurrency(bid.volume)}
                    </div>
                  </div>
                );
            ...
            {asks.map((ask, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="text-[8px] text-red-500 font-bold scale-90">
                    {formatCurrency(ask.price)}
                  </span>

                  <div className="absolute bottom-full mb-1 hidden group-hover:block bg-[#141923] border border-[#2d3748]/60 text-[8px] text-white p-1 rounded z-30 shadow-xl whitespace-nowrap">
                    Bán: {formatCurrency(ask.price)} | KL:{" "}
                    {formatCurrency(ask.volume)}
                  </div>
                </div>
              );
