...
import { X, Loader2, Sparkles, AlertTriangle, Calendar, Newspaper, CalendarRange } from 'lucide-react';
...
export const TickerDetailModal: React.FC<TickerDetailModalProps> = ({ symbol, isOpen, onClose }) => {
  ...
  }, [isOpen, loading, errorMsg, activeTab, timeframe, symbol]);
                    ...
                    <div className="flex gap-2">
                      {(['1m', '5m', '15m', '1D', '1W'] as const).map(tf => (
                        <button
                          ...
                          className={`border-0 py-0.5 px-1.5 rounded text-[9px] font-bold cursor-pointer transition-colors ${timeframe === tf ? 'bg-[#00c58e] text-slate-900 font-extrabold' : 'bg-white/5 text-[#7b8a9b] hover:text-white'}`}
                        >
                          {tf}
                        </button>
                      ))}
