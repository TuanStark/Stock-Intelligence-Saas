...
import { X, TrendingUp, Loader2, Sparkles, Activity, CalendarRange, Search, Plus, ChevronDown, Settings, Maximize2, Camera, Check, Cloud } from 'lucide-react';
...
export const TickerDetailPanel: React.FC<TickerDetailPanelProps> = ({ symbol, isOpen, onClose }) => {
  ...
  }, [isOpen, loading, errorMsg, activeSubTab, timeframe, currentSymbol, chartType, compareSymbol]);
      ...
      <div className="flex justify-between items-center p-3 border-b border-border-board bg-board-header">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-[#090b11] p-1 px-2.5 rounded border border-[#1b2233] text-[11px]">
            ...
            <span className="font-extrabold text-[#00c58e] tracking-tight">{currentSymbol.toUpperCase()}</span>
            <span className="text-[9px] text-text-muted font-bold bg-white/5 px-0.5 rounded uppercase">HOSE</span>
          </div>
          <span className="text-[10px] text-text-secondary font-bold truncate max-w-[120px]">{getCompanyName(currentSymbol)}</span>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={() => alert(`Đặt lệnh nhanh mã ${currentSymbol.toUpperCase()} trên Sidebar.`)}
                  ...
                  {showSearchDropdown && searchResults.length > 0 && (
                    <div className="absolute top-full left-0 mt-1.5 w-48 max-h-48 bg-[#1e222d] border border-[#2d313e] rounded shadow-2xl overflow-y-auto z-[200] p-1 scrollbar-none animate-scale-up">
                      {searchResults.map((item) => (
                        ...
                        >
                          <div>
                            <span className="font-extrabold text-[#00c58e] text-[10px] mr-1.5">{item.symbol}</span>
                            <span className="text-[9px] text-text-secondary truncate max-w-[80px] inline-block">{item.name}</span>
                          </div>
                          <span className="text-[8px] text-[#848e9c] bg-[#141823] px-0.5 rounded uppercase font-bold">HOSE</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="relative flex items-center h-full">
                  ...
                  {isCompareOpen && (
                    <div className="absolute top-full left-0 mt-1.5 w-40 bg-[#1e222d] border border-[#2d313e] rounded shadow-2xl p-1.5 z-[200] flex flex-col gap-1">
                      <span className="text-[8.5px] text-[#848e9c] font-extrabold uppercase">So sánh</span>
                ...
                <div className="relative flex items-center h-full">
                  ...
                  {isSettingsOpen && (
                    <div className="absolute top-full right-0 mt-1.5 w-36 bg-[#1e222d] border border-[#2d313e] rounded shadow-2xl p-1.5 z-[200] flex flex-col gap-1.5">
                      <span className="text-[8.5px] text-[#848e9c] font-extrabold uppercase pb-0.5 border-b border-[#2d313e]">Chỉ báo</span>
