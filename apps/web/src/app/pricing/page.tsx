...
export default function PricingPage() {
  ...
  const [selectedProvider, setSelectedProvider] = useState<"PAYOS" | "SEPAY">(
    "PAYOS",
  );
  const [paymentData, setPaymentData] = useState<any>(null);
  const [paymentStep, setPaymentStep] = useState<"SELECT_PROVIDER" | "SHOW_QR">(
    "SELECT_PROVIDER",
  );
  ...
  const handleInitializePayment = async (e: React.FormEvent) => {
    ...
    if (!session) {
      router.push(
        `/login?callbackUrl=/pricing?tier=${checkoutTier}&provider=${selectedProvider}`,
      );
      return;
    }
    ...
    try {
      const result = await authApi.upgradeSubscription(
        checkoutTier,
        selectedProvider,
      );
  ...
  const handleConfirmTransfer = async () => {
    ...
    try {
      ...
      const statusCheck = await authApi.checkTransactionStatus(
            ...
            {paymentSuccess ? (
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div>
                      <h4 className="font-extrabold text-sm text-text-primary">
                        {locale === "vi"
                          ...
                          : "PayOS gateway (VietQR)"}
                      </h4>
                      <p className="text-text-secondary text-xs mt-1 leading-relaxed">
                        {locale === "vi"
                          ? "Quét mã VietQR điền sẵn số tiền và nội dung chuyển khoản qua ứng dụng ngân hàng."
                          : "Scan VietQR code with pre-filled amount and content."}
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-extrabold text-sm text-text-primary">
                        {locale === "vi"
                          ? "Chuyển khoản trực tiếp (SePay)"
                          : "Direct Bank Transfer (SePay)"}
                      </h4>
                      <p className="text-text-secondary text-xs mt-1 leading-relaxed">
                        {locale === "vi"
                          ? "Tự động kiểm soát giao dịch qua cú pháp chuyển khoản ngân hàng chính xác."
                          : "Track payment automatically via exact transfer syntax."}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-5 relative z-10">
                ...
                {paymentData?.qrUrl && (
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-text-muted text-[10px] mt-2 italic text-center max-w-[280px]">
                      {locale === "vi"
                        ? "* Quét mã QR bằng ứng dụng ngân hàng (Mobile Banking) để tự động điền thông tin"
                        : "* Scan QR with your Mobile Banking app to prefill transaction info"}
                    </p>
                  </div>
                )}
                ...
                <div className="bg-surface border border-board-border rounded-xl p-4 flex flex-col gap-3">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-text-muted">
                      {locale === "vi" ? "Cổng thanh toán:" : "Gateway:"}
                    </span>
                    <span className="font-bold text-accent text-right uppercase">
                  ...
                  <div className="flex justify-between items-center text-xs border-t border-board-border/40 pt-2">
                    ...
                    <span
                      className="font-mono font-extrabold text-warning bg-surface-hover border border-board-border px-2 py-0.5 rounded text-right tracking-wider select-all cursor-pointer"
                      title={
                        locale === "vi" ? "Bấm để chọn" : "Click to select"
                      }
                    >
                      {paymentData?.referenceCode}
                    </span>
                ...
                <p className="text-xs text-text-secondary leading-relaxed bg-accent/5 border border-accent/20 rounded-xl p-3.5">
                  ...
                  {paymentData?.transferInstructions}
                </p>
                ...
                {paymentData?.paymentUrl && (
                  <div className="text-center">
                    ...
                    >
                      {locale === "vi"
                        ? "Mở trang thanh toán cổng "
                        : "Open payment link on "}
                      {paymentData.provider} &rarr;
                    </a>
                  </div>
                )}
