...
export function useStockChartDrawing() {
  ...
  const handleChartClick = (
    ...
    if (isLockedRef.current) {
      setDrawStatus(
        "Hình vẽ đã bị khóa! Hãy mở khóa (Unlock) trên thanh công cụ để tiếp tục vẽ.",
      );
    ...
    if (isMagnetRef.current && rawCandles.length > 0) {
      const candle = rawCandles.find((c) => {
        ...
        if (
          ...
          return (
            c.time.year === param.time.year &&
            c.time.month === param.time.month &&
            c.time.day === param.time.day
          );
        }
    ...
    // 🔱 Tool 2: Fibonacci Retracement
      ...
      if (drawingStepRef.current === 0) {
        ...
        setDrawingStep(1);
        setDrawStatus(
          "Click điểm thứ hai (Swing High/Low) để hoàn tất tỷ lệ Fibonacci",
        );
      } else if (drawingStepRef.current === 1 && drawingPoint1Ref.current) {
        const p1 = drawingPoint1Ref.current;
        const p2 = { time: param.time, price: activePrice };
    ...
    // 🟩 Tool 3: Shapes (Rectangle Zone)
      ...
      if (drawingStepRef.current === 0) {
        ...
        topLine.setData([
          { time: p1.time, value: topPrice },
          { time: p2.time, value: topPrice },
        ]);
        ...
        bottomLine.setData([
          { time: p1.time, value: bottomPrice },
          { time: p2.time, value: bottomPrice },
        ]);
        trendlineSeriesArrayRef.current.push(bottomLine);

        const leftLine = chart.addSeries(LineSeries, {
          color: zoneColor,
          lineWidth: 1,
          lineStyle: 2,
          title: "Zone Left",
        ...
        const rightLine = chart.addSeries(LineSeries, {
          color: zoneColor,
          lineWidth: 1,
          lineStyle: 2,
          title: "Zone Right",
    ...
    // 💬 Tool 4: Text Annotation
      ...
      const userText = prompt(
        "Nhập nội dung ghi chú/chú thích tại vị trí nến này:",
      );
    ...
    // 📏 Tool 5: Ruler
      ...
      if (drawingStepRef.current === 0) {
        ...
        if (rawCandles.length > 0) {
          const idx1 = rawCandles.findIndex(
            (c) => JSON.stringify(c.time) === JSON.stringify(p1.time),
          );
          const idx2 = rawCandles.findIndex(
            (c) => JSON.stringify(c.time) === JSON.stringify(p2.time),
          );
          if (idx1 !== -1 && idx2 !== -1) {
        ...
        alert(
          `📏 ĐO LƯỜNG PHÂN TÍCH KHOẢNG GIÁ & THỜI GIAN:\n\n` +
    ...
    // 🔍 Tool 6: Zoom Range
      ...
      if (drawingStepRef.current === 0) {
        ...
        setDrawingStep(1);
        setDrawStatus(
          "Click điểm thứ hai để hoàn tất vùng thời gian thu phóng",
        );
      } else if (drawingStepRef.current === 1 && drawingPoint1Ref.current) {
        const p1 = drawingPoint1Ref.current;
        const p2 = { time: param.time, price: activePrice };
