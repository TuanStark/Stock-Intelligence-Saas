...
export function useStockWebSocket(
  ...
  basePrice: number,
  onTick?: (tick: { price: number; change: number; changePercent: number; timestamp: string }) => void
) {
  ...
  useEffect(() => {
    ...
    function generateMockTradesHistory(price: number, base: number) {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const tradePrice = Math.max(Math.round(base * 0.93), Math.min(Math.round(base * 1.07), price + priceOffset));
