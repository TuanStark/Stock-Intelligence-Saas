...
export const marketApi = {
  ...
  search: async (query: string): Promise<any> => {
    return apiClient.get(
      `/market/instruments/search?q=${encodeURIComponent(query)}`,
    );
  },
  ...
  getSignals: async (type?: string): Promise<any> => {
    const url =
      type && type !== "ALL"
        ? `/market/signals?type=${type}`
        : "/market/signals";
    return apiClient.get(url);
  },
  triggerAiSummary: async (symbol: string): Promise<any> => {
    return apiClient.post(
      `/market/instruments/${symbol.toUpperCase()}/ai-summary`,
      {},
    );
  },
  getFinancials: async (symbol: string): Promise<any> => {
    return apiClient.get(
      `/market/instruments/${symbol.toUpperCase()}/financials`,
    );
  },
};
