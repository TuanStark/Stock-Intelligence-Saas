...
export const env = {
  ...
  API_SIGN_SECRET:
    process.env.API_SIGN_SECRET || 'stockintel-secret-hmac-key-2026',
  API_ENCRYPTION_KEY:
    process.env.API_ENCRYPTION_KEY || 'stockintel-aes-key-must-be-32bytes',
...
if (env.NODE_ENV === 'production') {
  ...
  if (
    ...
    env.API_SIGN_SECRET === 'stockintel-secret-hmac-key-2026'
  ) {
    missingVars.push('API_SIGN_SECRET (missing or using unsafe default value)');
  }
  if (
    !process.env.API_ENCRYPTION_KEY ||
    env.API_ENCRYPTION_KEY === 'stockintel-aes-key-must-be-32bytes'
  ) {
    missingVars.push(
      'API_ENCRYPTION_KEY (missing or using unsafe default value)',
    );
  }
  ...
  if (
    ...
    env.PAYMENT_BANK_ACCOUNT === '1133224455' ||
    env.PAYMENT_BANK_ACCOUNT === '990022884466'
  ) {
    missingVars.push(
      'PAYMENT_BANK_ACCOUNT (missing or using unsafe default mock account number)',
    );
  }
