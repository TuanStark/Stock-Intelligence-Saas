...
export class SubscriptionService {
  ...
  async initiateUpgrade(userId: string, tier: string, providerStr: string) {
    ...
    if (upperTier !== 'PRO' && upperTier !== 'API') {
      throw new BadRequestException(
        `Invalid subscription upgrade tier: ${tier}`,
      );
    }
    ...
    if (upperProvider !== 'PAYOS' && upperProvider !== 'SEPAY') {
      throw new BadRequestException(
        `Unsupported payment provider: ${providerStr}`,
      );
    }
    const provider = upperProvider;
    ...
    const bankId =
      env.PAYMENT_BANK_ID || (provider === 'PAYOS' ? 'vietinbank' : 'mb');
    const bankAccount =
      env.PAYMENT_BANK_ACCOUNT ||
      (provider === 'PAYOS' ? '1133224455' : '990022884466');
    const bankName =
      env.PAYMENT_BANK_NAME ||
      (provider === 'PAYOS'
        ? 'STOCK INTELLIGENCE'
        : 'STOCK INTELLIGENCE SEPAY');
    ...
    this.logger.log(
      `Tạo giao dịch thanh toán thành công: ${referenceCode} cho User: ${userId}`,
    );
  ...
  async handlePayosWebhook(payload: any, signature: string) {
    ...
    if (
      ...
      this.logger.warn(
        `⚠️ [PayOS Webhook] Phát hiện đang sử dụng Webhook Secret mặc định trong môi trường Development. BỎ QUA xác thực chữ ký để tạo thuận lợi cho việc test!`,
      );
    ...
    this.logger.log(
      `Đã nhận webhook PayOS hợp lệ cho đơn hàng ${referenceCode} (TxID: ${providerTxId})`,
    );
  ...
  async handleSepayWebhook(payload: any, apiKeyHeader: string) {
    ...
    this.logger.log(
      `Đã nhận webhook SePay hợp lệ cho đơn hàng ${referenceCode}`,
    );
  ...
  async checkTransactionStatus(userId: string, referenceCode: string) {
    ...
    if (tx.userId !== userId) {
      throw new UnauthorizedException(
        'Không có quyền truy cập thông tin giao dịch này',
      );
    }
